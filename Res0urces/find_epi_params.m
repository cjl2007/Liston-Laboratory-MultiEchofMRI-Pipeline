...
for s = 1:length(sessions)
    ...
    for r = 1:length(runs)
        ...
        for e = 1:length(json)
            ...
            % echo times;
            te(count,e) = j.EchoTime*10^3; % convert to ms
        ...
        system(['echo ' num2str(te(count,:)) ' > ' Subdir '/func/rest/session_' num2str(s) '/run_' num2str(r) '/te.txt']);
...
for s = 1:length(sessions)
    ...
    for r = 1:length(runs)
        ...
        count = count + 1; % tick
        system(['echo Session ' num2str(s) ' Run ' num2str(r) ': ' num2str(te(count,:)) ' >> ' Subdir '/func/qa/AllTEs.txt']);

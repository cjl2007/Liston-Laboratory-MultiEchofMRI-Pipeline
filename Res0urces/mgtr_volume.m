...
info_3D = niftiinfo([Subdir '/func/xfms/rest/T1w_acpc_brain_2mm_mask.nii.gz']); % nii info; used for writing the output.
...
if ~exist([Subdir '/anat/T1w/ROIs/CorticalRibbon.nii.gz'],'file')
    str = strsplit(Subdir,'/'); Subject = str{end}; % infer subject name
    system(['mri_convert -i ' Subdir '/anat/T1w/' Subject '/mri/lh.ribbon.mgz -o ' Subdir '/anat/T1w/ROIs/lh.ribbon.nii.gz --like ' Subdir '/func/xfms/rest/T1w_acpc_brain_2mm_mask.nii.gz > /dev/null 2>&1']);
    system(['mri_convert -i ' Subdir '/anat/T1w/' Subject '/mri/rh.ribbon.mgz -o ' Subdir '/anat/T1w/ROIs/rh.ribbon.nii.gz --like ' Subdir '/func/xfms/rest/T1w_acpc_brain_2mm_mask.nii.gz > /dev/null 2>&1']);
    system(['fslmaths ' Subdir '/anat/T1w/ROIs/lh.ribbon.nii.gz -add ' Subdir '/anat/T1w/ROIs/rh.ribbon.nii.gz ' Subdir '/anat/T1w/ROIs/CorticalRibbon.nii.gz > /dev/null 2>&1']);
    system(['fslmaths ' Subdir '/anat/T1w/ROIs/CorticalRibbon.nii.gz -bin ' Subdir '/anat/T1w/ROIs/CorticalRibbon.nii.gz']);
    system(['rm ' Subdir '/anat/T1w/ROIs/*.ribbon.*']); % clean up;
end

% read in mask and define all "in-brain" voxels;
gray = niftiread([Subdir '/anat/T1w/ROIs/CorticalRibbon.nii.gz']);

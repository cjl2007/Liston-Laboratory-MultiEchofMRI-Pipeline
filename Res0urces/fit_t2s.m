...
info = niftiinfo([Subdir '/func/xfms/rest/T1w_acpc_brain_func_mask.nii.gz']);
...
for s = 1:length(sessions)
    ...
    for r = 1:length(runs)
        ...
        % load echo times;
        te = load([Subdir '/func/rest/session_' num2str(s) '/run_' num2str(r) '/TE.txt']); % lets assume that all scans have the same te; this may not be true in practice; user beware.
        ...
        for i = 1:length(te)
            ...
            % read in the .nii and remove intermediate file;
            nii = niftiread([Subdir '/workspace/temp_e' num2str(i) '_s' num2str(s) '_r' num2str(r) '.nii.gz']);
            system(['rm ' Subdir '/workspace/temp_e' num2str(i) '_s' num2str(s) '_r' num2str(r) '.nii.gz']);
...
% fill in bad values;
system(['wb_command -volume-dilate ' Subdir '/func/t2star/T2star_acpc.nii.gz 10 NEAREST  ' Subdir '/func/t2star/T2star_acpc.nii.gz -data-roi ' Subdir '/func/xfms/rest/T1w_acpc_brain_func.nii.gz']);

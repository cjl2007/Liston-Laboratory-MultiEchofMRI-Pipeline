...
mask = niftiread([Subdir '//func/xfms/rest/T1w_nonlin_brain_2mm_mask.nii.gz']);
...
info = niftiinfo([Subdir '/func/xfms/rest/T1w_nonlin_brain_2mm_mask.nii.gz']);
...
for s = 1:length(sessions)
    ...
    for r = 1:length(runs)
        ...
        parfor i = 1:length(te)
            ...
            system(['fslmaths ' Subdir '/func/rest/session_' num2str(s) '/run_' num2str(r) '/Rest_E' num2str(i) '_nonlin.nii.gz -Tmean ' Subdir '/workspace/temp_e' num2str(i) '_s' num2str(s) '_r' num2str(r) '.nii.gz']); 
...
niftiwrite(t2s,[Subdir '/func/t2star/T2star_nonlin'],info);
system(['gzip -f ' Subdir '/func/t2star/T2star_nonlin.nii']);

% reshape, write, and compress t2s map;
gof = reshape(gof,[dims(1),dims(2),dims(3)]);
niftiwrite(gof,[Subdir '/func/t2star/R2_nonlin'],info);
system(['gzip -f ' Subdir '/func/t2star/R2_nonlin.nii']);

% fill in bad values;
system(['wb_command -volume-dilate ' Subdir '/func/t2star/T2star_nonlin.nii.gz 10 NEAREST  ' Subdir '/func/t2star/T2star_nonlin.nii.gz -data-roi ' Subdir '/func/xfms/rest/T1w_nonlin_brain_2mm.nii.gz']);

% now, oc-me

% create and load a brain mask;
t2s = niftiread([Subdir '/func/t2star/T2star_nonlin.nii.gz']);

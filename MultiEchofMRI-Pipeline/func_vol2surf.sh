...
for s in $sessions ; do
	...
	for r in $runs ; do
		...
		for c in $CiftiList ; do
			...
			wb_command -cifti-create-dense-timeseries "$OUT_DIR"/Rest_"$c".dtseries.nii -volume "$Subdir"/func/rest/session_"$s"/run_"$r"/Rest_"$c".nii.gz "$Subdir"/anat/T1w/ROIs/Atlas_ROIs.2.nii.gz \

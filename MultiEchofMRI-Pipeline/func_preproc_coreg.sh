...
# create subcortical label file &
# downsample to 2mm acpc atlas space;
mkdir "$Subdir"/anat/T1w/ROIs > /dev/null 2>&1 
mri_label2vol --seg "$Subdir"/anat/T1w/"$Subject"/mri/aseg.mgz --temp "$Subdir"/anat/T1w/"$Subject"/mri/rawavg.mgz --o "$Subdir"/anat/T1w/"$Subject"/mri/aseg-in-rawavg.mgz --regheader "$Subdir"/anat/T1w/"$Subject"/mri/aseg.mgz > /dev/null 2>&1
mri_convert -i "$Subdir"/anat/T1w/"$Subject"/mri/aseg-in-rawavg.mgz -o "$Subdir"/anat/T1w/"$Subject"/mri/aseg-in-rawavg.nii.gz --like "$Subdir"/anat/T1w/T1w_acpc_dc_restore.nii.gz > /dev/null 2>&1
flirt -interp nearestneighbour -in "$Subdir"/anat/T1w/"$Subject"/mri/aseg-in-rawavg.nii.gz -ref "$T1wTemplate2mm" -applyxfm -init "$RESOURCES"/ident.mat -out "$Subdir"/anat/T1w/ROIs/Atlas_ROIs.2.nii.gz
wb_command -volume-label-import "$Subdir"/anat/T1w/ROIs/Atlas_ROIs.2.nii.gz "$RESOURCES"/FS/SubcorticalLabels.txt "$Subdir"/anat/T1w/ROIs/Atlas_ROIs.2.nii.gz -discard-others > /dev/null 2>&1

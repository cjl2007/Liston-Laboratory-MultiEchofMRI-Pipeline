...
for s in $sessions ; do
	...
	for r in $runs ; do
		...
		for i in OCME OCME+MEICA OCME+MEICA+MGTR ; do
			...
			wb_command -cifti-smoothing "$Subdir"/func/rest/session_"$s"/run_"$r"/Rest_"$i".dtseries.nii 2.55 2.55 COLUMN "$Subdir"/func/rest/session_"$s"/run_"$r"/Rest_"$i"_s2.55.dtseries.nii \

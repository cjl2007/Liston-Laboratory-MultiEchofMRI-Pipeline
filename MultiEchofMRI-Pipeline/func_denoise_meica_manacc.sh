...
for s in $sessions ; do
	...
	for r in $runs ; do
		...
		echo tdir=["'$Subdir/func/rest/session_$s/run_$r/Tedana/'"] | cat - "$Subdir"/workspace/temp.m >> temp && mv temp "$Subdir"/workspace/temp.m > /dev/null 2>&1  
...
func () {
	...
	--mix "$2"/func/rest/"$3"/Tedana/ica_mixing.tsv --ctab "$2"/func/rest/"$3"/Tedana/ica_decomposition.json --manacc $(cat "$2"/func/rest/"$3"/Tedana/man_acc.txt) \
